function Invoke-IamSessionApiIamSessionsGet {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Calling method: IamSessionApi-IamSessionsGet' | Write-Verbose
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $Script:IamSessionApi.IamSessionsGet(
            ${$count},
            ${$inlinecount},
            ${$top},
            ${$skip},
...
function Invoke-IamSessionApiIamSessionsMoidDelete {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Calling method: IamSessionApi-IamSessionsMoidDelete' | Write-Verbose
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $Script:IamSessionApi.IamSessionsMoidDelete(
            ${moid},
            ${moid2},
            ${moid3},
            ${moid4},
...
function Invoke-IamSessionApiIamSessionsMoidGet {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $true)]
        [String]
    ...
    Process {
        'Calling method: IamSessionApi-IamSessionsMoidGet' | Write-Verbose
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $Script:IamSessionApi.IamSessionsMoidGet(
            ${moid},
            ${moid2},
            ${moid3},
            ${moid4},
